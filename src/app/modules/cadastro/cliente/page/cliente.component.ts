...
import { TipoDocumento } from '../../../../models/Interfaces/cadastro/clientes/TipoDocumento.interface';
import { DropdownUfOptions } from '../../../../models/Interfaces/cadastro/clientes/DropdownUfOptions.interface';
import { Uf } from '../../../../models/Enum/cadastro/Uf.enum';
...
export class ClienteComponent implements OnInit {
  ...
  constructor(
     ...
     */
    this.clienteForm = this.formBuilderCliente.group({
  ...
  onEditButtonClick(cliente: Clientes): void {
    ...
    if (cliente.status === 'DESATIVADO') {
      ...
      this.clienteService.getCliente(cliente.codigo).subscribe((data) => {
        this.clienteForm.patchValue({
          codigo: data.codigo,
  ...
  disableSelectedClientes() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.clienteDatas = this.clienteDatas.filter(
          (val) => !this.clienteSelecionado?.includes(val),
        );
        this.clienteSelecionado = null;
        this.messageService.add({
          severity: 'success',
          summary: 'Sucesso',
          detail: 'Usuarios Excluídos',
          life: 3000,
        });
      },
    });
  ...
  getDocumentoMask(): string {
    ...
    return tipo === 'CPF' ? '999.999.999-99' : tipo === 'CNPJ' ? '99.999.999/9999-99' : '';
  }
